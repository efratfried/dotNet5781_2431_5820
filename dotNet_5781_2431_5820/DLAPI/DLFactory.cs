...
namespace DLAPI
{
    ...
    public static class DLFactory
    {
        ...
        {
            Assembly.Load("DLXML");
            Type type = Type.GetType($"DL.DLXML, DLXML", true);
            return type.GetProperty("Instance", BindingFlags.Public | BindingFlags.Static).GetValue(null) as IDL;
            ...
            try
            {
                type1 = Type.GetType($"{dlNameSpace}.{dlClass}, {dlPackageName}", true);
            }
            catch (Exception ex)
            ...
            try
            {
                IDL dal = type1.GetProperty("Instance", BindingFlags.Public | BindingFlags.Static).GetValue(null) as IDL;
